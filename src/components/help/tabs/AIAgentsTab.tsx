  ...
  return (
    <div className="space-y-8">
      <div className="glass rounded-xl p-6">
        <h2 className="font-sora text-2xl text-white mb-1">Built for AI Agents</h2>
        <p className="text-slate-400 text-sm">Every artifact is designed so an AI coding agent can implement, extend, or audit OneView without human explanation.</p>
      </div>
        ...
        {[
          ...
          <a key={a.file} href={a.link} target="_blank" rel="noopener noreferrer" className="glass glass-hover rounded-xl p-5 group flex flex-col gap-2 transition-all" style={{borderLeft:`3px solid ${a.color}`}}>
            <div className="flex items-center justify-between">
              <span className="font-mono font-medium text-slate-200 text-sm">{a.file}</span>
      ...
      <div className="glass rounded-xl p-6">
        <h3 className="font-sora text-lg text-slate-200 mb-4">Agent Quickstart (5 steps)</h3>
          ...
          {[
            ...
            <div key={s.step} className="flex gap-3">
              ...
              <div className="flex-1">
                <div className="text-slate-300 text-sm font-medium mb-1">{s.title}</div>
                <pre className="bg-black/40 rounded-lg p-2.5 text-xs text-emerald-300 font-mono whitespace-pre-wrap overflow-x-auto">{s.code}</pre>
              </div>
            </div>
          ))}
        </div>
