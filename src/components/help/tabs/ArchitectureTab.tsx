...
const LAYERS = [
  ...
  { id: "data", label: "Data + Privacy", nodes: [
    { id:"postgres", name:"Vercel Postgres", icon:"🗄️", color:"#10b981", desc:"Neon-backed Postgres via @vercel/postgres. 8 tables: clients, enrolments, outcomes, surveys, case_notes, placements, audit_log, report_cache.", steps:["CREATE TABLE IF NOT EXISTS for all 8 tables","gen_random_uuid() for PKs","Cascading deletes for referential integrity","Auto-seed 19,140 demo clients on first load"], code:'clients → enrolments\n  → outcomes | surveys\n  → placements\ncase_notes\naudit_log | report_cache' },
