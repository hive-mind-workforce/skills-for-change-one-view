...
export default function NarrativePanel({ narrative, funder, period, cached, loading }: NarrativePanelProps) {
  return (
    <div className="glass rounded-xl overflow-hidden">
      <div className="px-5 py-3 border-b border-slate-100 dark:border-white/[0.08] flex items-center justify-between" style={{background:"linear-gradient(90deg,rgba(16,185,129,0.1),transparent)"}}>
        <div>
      ...
      <div className="p-5">
        {loading ? (
          <div className="space-y-3 animate-pulse">
            {[...Array(4)].map((_,i) => <div key={i} className="h-4 bg-slate-200 dark:bg-white/[0.06] rounded" style={{width:`${[100,85,92,70][i]}%`}} />)}
          </div>
        ) : (
          <p className="text-slate-600 dark:text-slate-300 leading-relaxed text-sm whitespace-pre-wrap">{narrative}</p>
        )}
      </div>
    </div>
  )
