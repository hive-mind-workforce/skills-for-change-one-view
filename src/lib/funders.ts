...
export const FUNDERS: Record<string, FunderConfig> = {
  ...
  uw: {
    ...
    color: "#f59e0b",
    csvHeaders: ["Case ID","Client Name","Primary Language","Program","Enrollment Date","Sessions Completed","Outcomes Achieved","Support Type"],
    mapRow: (c, e, o) => ({
      "Case ID": c.id?.substring(0,8) ?? "",
      "Client Name": c.full_name ?? "",
      "Primary Language": c.primary_language ?? "",
      "Program": e.program ?? "",
      "Enrollment Date": e.enrolled_at ? new Date(e.enrolled_at).toISOString().split("T")[0] : "",
